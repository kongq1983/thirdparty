 ...
 *  setMessageListener  topic 每个监听器都能收到
 * @author kq
 * @date 2019-08-01
 */
public class ClientAcknowledgeCustomerTopicListenerDemo {
            ...
            if (message instanceof TextMessage) {
                ...
                try {
                    ...
                    if (StringUtils.startsWith(text, "acknowledge")) {
                        System.out.println(" -- topic确认调用acknowledge ");
                        // Session.CLIENT_ACKNOWLEDGE 模式 , 需要调用acknowledge ,否则队列不会清除
                        message.acknowledge();
                    } else {
                        //通知mq进行重发  最多重发六次  总共7次
                        System.out.println(" topic 调用recover ");
                        session.recover();
                    }
                    System.out.println(" Acknowledge topic 订阅者一收到的消息："+text );
                }catch (Exception e) {
                    e.printStackTrace();
                }
